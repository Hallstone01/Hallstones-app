...
const TRANSLATIONS = {
  ...
  pl: {
    title: "Wydarzenia i zloty",
    subtitle: "PROWINCJONALNE I KRAJOWE",
    upcoming: "NADCHODZĄCE",
    nothingYet: "Brak zaplanowanych wydarzeń.",
...
export default function EventsScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: rsvpData, error: rsvpError } = await supabase.from("event_rsvps").select("event_id, name");
      if (!cancelled && !rsvpError) {
        const grouped = {};
        (rsvpData || []).forEach((r) => {
          grouped[r.event_id] = grouped[r.event_id] || [];
          grouped[r.event_id].push(r.name);
        });
  ...
  async function submitRsvp(eventId) {
    ...
    setSubmitting(true);
    const { error } = await supabase.from("event_rsvps").insert({ event_id: eventId, name, email });
    setSubmitting(false);
    if (!error) {
      setMyRsvps((m) => ({ ...m, [eventId]: true }));
      setAttendees((a) => ({ ...a, [eventId]: [...(a[eventId] || []), name] }));
      setRsvpForm(null);
      setName("");
      setEmail("");
    } else {
